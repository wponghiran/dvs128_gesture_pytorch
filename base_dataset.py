...
def integrate_events_by_frame_duration(events: Dict, frame_duration: int, H: int, W: int) -> np.ndarray:
    events_t, x, y, p = (events[key] for key in ('t', 'x', 'y', 'p'))

    # Process similar to integrate_by_fixed_frame_number but compute num_frames based on frame_duration
    num_frames = ((events_t[-1] - events_t[0]) // frame_duration) + 1

    j_l = np.zeros(shape=[num_frames], dtype=int)
    j_r = np.zeros(shape=[num_frames], dtype=int)
    N = events_t.size

    idx = np.arange(N)
    for i in range(num_frames):
        t_l = frame_duration * i + events_t[0]
        t_r = t_l + frame_duration
        mask = np.logical_and(events_t >= t_l, events_t < t_r)
        idx_masked = idx[mask]
        j_l[i] = idx_masked[0]
        j_r[i] = idx_masked[-1] + 1
    ...
    frames = np.zeros([num_frames, 2, H, W])
    for i in range(num_frames):
        frames[i] = integrate_events_segment_to_frame(x, y, p, H, W, j_l[i], j_r[i])
    return frames
...
class NeuromorphicDatasetFolder(DatasetFolder):
    def __init__(
            ...
            split_by: str = None,   # add 'frame_duration'
            duration: int = None,
        ...
        if data_type == 'event':
            ...
            if frames_number is not None or frame_duration is not None:
                assert split_by in ['time', 'number', 'frame_duration'], f'{split_by} is invalid split option'
                if split_by == 'frame_duration':            
                    frames_np_root = os.path.join(root, f'split_by_fduration_{frame_duration}')
                else:
                ...
                if os.path.exists(frames_np_root):
                    print(f'The directory [{frames_np_root}] already exists. Use existing pre-processed dataset.')
                else:
                    os.mkdir(frames_np_root)
                    print(f'Make directory [{frames_np_root}]. Pre-process dataset.')
